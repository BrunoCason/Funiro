const Sec5Funiro = () => {
  return ( 
    <main className="container mx-auto text-center">
      <p className="font-poppins font-semibold text-Gray2 text-base lg:text-xl pt-14">
        Share your setup with
      </p>
      <h2 className="font-poppins font-bold text-Gray1 text-3xl lg:text-4.5xl">
        #FuniroFurniture
      </h2>
      <img
        className="container mx-auto lg:-mt-5 pr-5 pl-5 lg:pr-0 lg:pl-0"
        src="https://desafio-03-compass-uol.s3.us-east-2.amazonaws.com/static-images/Images-sec5.png"
        alt=""
      />
      <hr className="mt-10 mb-3 border-D9D9D9" />
    </main>
  );
};

export default Sec5Funiro;
